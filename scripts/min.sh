...
# @@File             :  min.sh
# @@Description      :  Script to setup min for Arch
...
RELEASE_TYPE="$(. /etc/os-release 2>/dev/null; [[ " $ID_LIKE " == *centos* ]] && echo "arch")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
if ! grep --no-filename -sE -- '^ID=|^ID_LIKE=|^NAME=' /etc/*-release | grep -qiwE -- "arch"; then
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
__yum() {
	pacman --noconfirm "$@" &>/dev/null || return 1
}
...
__retrieve_version_file() { __grab_remote_file "https://github.com/casjay-base/arch/raw/main/version.txt" | head -n1 || echo "Unknown version"; }
__domain_name() {
	local d="" f=""
...
if type -P systemmgr >/dev/null 2>&1; then
	...
	__run_external "pacman -Sc --noconfirm"
fi
...
CFLAGS="-march=x86-64 -mtune=generic -O2 -pipe -fno-plt -fexceptions -Wp,-D_FORTIFY_SOURCE=3 -Wformat -Werror=format-security -fstack-clash-protection \
...
# __install_pkg cronie-noanacron  # skipped on arch
...
__run_external "pacman -Syu --noconfirm"
##################################################################################################################
...
##################################################################################################################
# __install_pkg awffull  # skipped on arch
# __install_pkg awstats  # skipped on arch
__install_pkg base
__install_pkg bash
__install_pkg bash-completion
# __install_pkg biosdevname  # skipped on arch
__install_pkg certbot
__install_pkg cockpit
# __install_pkg cockpit-packagekit  # skipped on arch
# __install_pkg cockpit-storaged  # skipped on arch
# __install_pkg cockpit-pcp  # skipped on arch
# __install_pkg cockpit-bridge  # skipped on arch
# __install_pkg cockpit-system  # skipped on arch
# __install_pkg cockpit-ws  # skipped on arch
__install_pkg coreutils
__install_pkg cowsay
__install_pkg cracklib
# __install_pkg cracklib-dicts  # skipped on arch
__install_pkg cronie
# __install_pkg cronie-noanacron  # skipped on arch
# __install_pkg crontabs  # skipped on arch
__install_pkg curl
__install_pkg ctags
...
# __install_pkg mod_maxminddb  # skipped on arch
# __install_pkg mod_perl  # skipped on arch
# __install_pkg mod_ssl  # skipped on arch
# __install_pkg mod_wsgi  # skipped on arch
# __install_pkg mod_proxy_html  # skipped on arch
# __install_pkg mod_proxy_uwsgi  # skipped on arch
__install_pkg mosh
# __install_pkg mrtg  # skipped on arch
# __install_pkg munin  # skipped on arch
# __install_pkg munin-common  # skipped on arch
# __install_pkg munin-node  # skipped on arch
__install_pkg ncurses
# __install_pkg ncurses-base  # skipped on arch
# __install_pkg ncurses-libs  # skipped on arch
__install_pkg net-tools
__install_pkg nginx
# __install_pkg oddjob-mkhomedir  # skipped on arch
...
# __install_pkg perl-DBD-Firebird  # skipped on arch
...
# __install_pkg php-cli $_php_install_opts  # skipped on arch
# __install_pkg php-common $_php_install_opts  # skipped on arch
...
# __install_pkg php-xml $_php_install_opts  # skipped on arch
unset _php_install_opts
__install_pkg pinentry
__install_pkg postfix
# __install_pkg postfix-pcre  # skipped on arch
__install_pkg certbot-dns-rfc2136
__install_pkg python-configargparse
__install_pkg python-cryptography
# __install_pkg python3-enum34  # skipped on arch
# __install_pkg python3-funcsigs  # skipped on arch
# __install_pkg python3-future  # skipped on arch
__install_pkg python-idna
# __install_pkg python3-josepy  # skipped on arch
# __install_pkg python3-mock  # skipped on arch
__install_pkg python-pynvim
# __install_pkg python3-parsedatetime  # skipped on arch
# __install_pkg python3-pbr  # skipped on arch
__install_pkg python-pip
__install_pkg python-psutil
# __install_pkg python3-pyasn1  # skipped on arch
# __install_pkg python3-pyrfc3339  # skipped on arch
# __install_pkg python3-pysocks  # skipped on arch
__install_pkg python-requests
# __install_pkg python3-six  # skipped on arch
__install_pkg python-virtualenv
__install_pkg readline
# __install_pkg rootfiles  # skipped on arch
__install_pkg rsync
# __install_pkg rsyslog  # skipped on arch
...
__install_pkg unzip
# __install_pkg webalizer  # skipped on arch
__install_pkg wget
__install_pkg which
__install_pkg whois
__install_pkg xz
...
if [ -d "/srv/http/sysinfo/.git" ]; then
	devnull git -C "/srv/http/sysinfo" reset --hard
	__run_post git -C "/srv/http/sysinfo" pull -q
else
	devnull __rm_if_exists "/srv/http/sysinfo"
	__run_post git clone -q "https://github.com/phpsysinfo/phpsysinfo" "/srv/http/sysinfo"
fi
if [ -d "/srv/http/vnstat/.git" ]; then
	devnull git -C "/srv/http/vnstat" reset --hard
	__run_post git -C "/srv/http/vnstat" pull -q
else
	devnull __rm_if_exists "/srv/http/vnstat"
	__run_post git clone -q "https://github.com/solbu/vnstat-php-frontend" "/srv/http/vnstat"
fi
...
if __system_service_active named || __port_in_use "53"; then
	devnull __rm_if_exists $CONFIG_TEMP_DIR/etc/named*
	devnull __rm_if_exists $CONFIG_TEMP_DIR/var/named*
else
	devnull __rm_if_exists /etc/named* /var/named/*
fi
...
if ! type -P httpd >/dev/null 2>&1; then
	...
	devnull __rm_if_exists /etc/httpd*
	devnull __rm_if_exists $CONFIG_TEMP_DIR/etc/httpd*
fi
...
if ! type -P named >/dev/null 2>&1; then
	...
	devnull __rm_if_exists /etc/named*
	devnull __rm_if_exists /var/named*
	devnull __rm_if_exists $CONFIG_TEMP_DIR/etc/named*
	devnull __rm_if_exists $CONFIG_TEMP_DIR/var/named*
fi
...
if [ -n "$le_primary_domain" ]; then
	...
	if [ -d "/etc/letsencrypt/live/$le_primary_domain" ] || [ -d "/etc/letsencrypt/live/domain" ]; then
		...
		find "/etc/postfix" "/etc/httpd" "/etc/nginx" /etc/proftpd* -type f -exec sed -i \
...
if type -P tor >/dev/null 2>&1; then
	...
	if [ -n "$tor_hostnames" ]; then
		devnull __rm_if_exists "/srv/http/tor_hostname"
		for f in $tor_hostnames; do
			cat "$f" >>"/srv/http/tor_hostname" 2>/dev/null
		done
	fi
...
if [ -n "$GET_WEB_USER" ]; then
	...
	if [ -f "/etc/php/fpm/pool.d/www.conf" ]; then
		sed -i '0,/^user .*/s//user = '$GET_WEB_USER'/' "/etc/php/fpm/pool.d/www.conf"
		grep -sqh -- "^user = $GET_WEB_USER" "/etc/php/fpm/pool.d/www.conf" || echo "Failed to change the user in /etc/php/fpm/pool.d/www.conf"
	fi
	if [ -f "/etc/httpd/conf/httpd.conf" ]; then
...
if [ -n "$GET_WEB_GROUP" ]; then
	if [ -f "/etc/php/fpm/pool.d/www.conf" ]; then
		sed -i '0,/^group .*/s//group = '$GET_WEB_GROUP'/' "/etc/php/fpm/pool.d/www.conf"
		grep -sqh -- "^group = $GET_WEB_GROUP" "/etc/php/fpm/pool.d/www.conf" || echo "Failed to change the group in /etc/php/fpm/pool.d/www.conf"
	fi
	if [ -f "/etc/httpd/conf/httpd.conf" ]; then
...
##################################################################################################################
mkdir -p "/mnt/backups" "/srv/http/.well-known" "/etc/letsencrypt/live"
echo "" >>/etc/fstab
if [ -n "$IS_NETWORK_INTERNAL" ] && devnull ping -q -W 1 -c 2 10.0.254.1; then
	{
		...
		echo "10.0.254.1:/srv/http/.well-known     /srv/http/.well-known    nfs defaults,rw 0 0"
	} >>/etc/fstab
fi
mount -a
##################################################################################################################
