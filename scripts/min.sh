...
retrieve_version_file() { grab_remote_file "https://github.com/casjay-base/arch/raw/main/version.txt" | head -n1 || echo "Unknown version"; }
domain_name() {
	local d="" f=""
...
if type -P systemmgr >/dev/null 2>&1; then
	...
	run_external "pacman -Sc --noconfirm"
fi
...
##################################################################################################################
# install_pkg awffull  # skipped on arch
# install_pkg awstats  # skipped on arch
install_pkg base
install_pkg bash
install_pkg bash-completion
# install_pkg biosdevname  # skipped on arch
install_pkg certbot
install_pkg cockpit
# install_pkg cockpit-packagekit  # skipped on arch
# install_pkg cockpit-storaged  # skipped on arch
# install_pkg cockpit-pcp  # skipped on arch
# install_pkg cockpit-bridge  # skipped on arch
# install_pkg cockpit-system  # skipped on arch
# install_pkg cockpit-ws  # skipped on arch
...
install_pkg gnutls
install_pkg grub
# install_pkg grub2-tools-extra  # skipped on arch
# install_pkg grubby  # skipped on arch
install_pkg gzip
...
# install_pkg perl-DBD-Firebird  # skipped on arch
...
# install_pkg python3-funcsigs  # skipped on arch
# install_pkg python3-future  # skipped on arch
install_pkg python-idna
# install_pkg python3-josepy  # skipped on arch
# install_pkg python3-mock  # skipped on arch
install_pkg python-pynvim
# install_pkg python3-parsedatetime  # skipped on arch
# install_pkg python3-pbr  # skipped on arch
install_pkg python-pip
install_pkg python-psutil
# install_pkg python3-pyasn1  # skipped on arch
# install_pkg python3-pyrfc3339  # skipped on arch
# install_pkg python3-pysocks  # skipped on arch
install_pkg python-requests
# install_pkg python3-six  # skipped on arch
...
install_pkg tzdata
install_pkg unzip
# install_pkg webalizer  # skipped on arch
install_pkg wget
install_pkg which
install_pkg whois
install_pkg xz
...
if [ -d "/srv/http/sysinfo/.git" ]; then
	...
	run_post git -C "/srv/http/sysinfo" pull -q
else
	devnull rm_if_exists "/srv/http/sysinfo"
	run_post git clone -q "https://github.com/phpsysinfo/phpsysinfo" "/srv/http/sysinfo"
fi
if [ -d "/srv/http/vnstat/.git" ]; then
	devnull git -C "/srv/http/vnstat" reset --hard
	run_post git -C "/srv/http/vnstat" pull -q
else
	devnull rm_if_exists "/srv/http/vnstat"
	run_post git clone -q "https://github.com/solbu/vnstat-php-frontend" "/srv/http/vnstat"
fi
...
if type -P tor >/dev/null 2>&1; then
	...
	if [ -n "$tor_hostnames" ]; then
		devnull rm_if_exists "/srv/http/tor_hostname"
