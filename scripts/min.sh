...
RELEASE_TYPE="$(
	...
	i="${ID,,}"
	l=" ${ID_LIKE,,} "
	if [ "$i" = "rhel" ] || [ "$i" = "arch" ] || [ "$i" = "ol" ] || [[ "$l" == *" rhel "* ]] || [[ "$l" == *" centos "* ]]; then
		echo ""
	fi
)"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
_oci_pkgs="$(pacman -Qq 2>/dev/null | grep -E -- '^(oci|cloud|oracle)')"
...
##################################################################################################################
find "/etc" "/usr" "/var" -iname '*.pacnew' -exec rm -Rf {} \; >/dev/null 2>&1
find "/etc" "/usr" "/var" -iname '*.pacsave' -exec rm -Rf {} \; >/dev/null 2>&1
