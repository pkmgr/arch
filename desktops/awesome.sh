...
install_aur() {
  if test_pkg "$1"; then execute "sudo --user=$(whoami) pkmgr install $1" "Installing: $1"; fi
  setexitstatus
  set --
}
...
##################################################################################################################
printf_head "Installing the awesome desktop"
##################################################################################################################
...
install_pkg arcolinux-awesome-git
install_pkg arcolinux-config-awesome-git
install_pkg arcolinux-awesome-dconf-git
...
[ ! -d "$HOME/.config/awesome" ] && [ -d "/etc/skel/.config/awesome" ] &&
  run_post "cp -rT /etc/skel/.config/awesome $HOME/.config/awesome"

run_post "dotfiles install awesome"
