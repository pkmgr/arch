...
# @Resource    : https://github.com/arcolinuxd/arco-awesome
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__system_service_disable() {
  if __system_service_exists; then __execute "sudo systemctl disable --now $1" "Disabling service: $1"; fi
  __setexitstatus
  set --
}
...
__install_aur() {
  if __test_pkg "$1"; then __execute "sudo --user=$(whoami) pkmgr install $1" "Installing: $1"; fi
  __setexitstatus
  set --
}
...
##################################################################################################################
__printf_head "Installing the awesome desktop"
##################################################################################################################

__install_pkg awesome
__install_pkg vicious

__install_pkg arcolinux-xfce-git
__install_pkg arcolinux-awesome-git
__install_pkg arcolinux-config-awesome-git
__install_pkg arcolinux-awesome-dconf-git
...
[ ! -d "$HOME/.config/awesome" ] && [ -d "/etc/skel/.config/awesome" ] &&
  __run_post "cp -rT /etc/skel/.config/awesome $HOME/.config/awesome"

__run_post "dotfiles install awesome"
